...
def problem_two(request):
    ...
    insturctors = Instructor.objects.filter(hire_date__year__lt=2010)#.order_by('hire_date')
...
def problem_three(request):
    ...
    print(
        f'Instructor Name: {instructor.first_name} {instructor.last_name}\n'
        'Courses:'
    )
...
def problem_five(request):
    ...
    new_student = Student.objects.create(first_name="Charles", last_name="Taylor", year=2022, gpa=5)

    print(
        f'Id: {new_student.id}\n'
        f'Full Name: {new_student.first_name} {new_student.last_name}\n'
        f'Year: {new_student.year}\n'
        f'GPA: {new_student.gpa}'
    )
    return complete(request)
...
def problem_six(request):
    ...
    student_id = 11
    Student.objects.filter(id=student_id).update(gpa=5.1)
    updated_student = Student.objects.get(id=student_id)

    print(
        f'Id: {updated_student.id}\n'
        f'Full Name: {updated_student.first_name} {updated_student.last_name}\n'
        f'GPA: {updated_student.gpa}'
    )
...
def bonus_problem(request):
    ...
    variable = Instructor.objects.annotate(Count('course')).filter(course__count=1)

    for var in variable:
        print(
            f'{var.first_name} {var.last_name}'
        )
